...
```{r}
# unclass(sigma(forc))["1990-01-16 08:00:00"]
x <- unclass(sigma(forc))[,"1990-01-16 08:00:00"][1:252]
x.level()
```
...
# # date[length(date)]
# next_year_date <- seq.Date(as.Date(date[length(date)])+1, length.out = 252, by = "day")
# forecast <- data.frame(
#   Date = next_year_date,
#   residuals = list(forc@forecast$sigmaFor)
# )
# names(forecast)[names(forecast) == "X1990.01.16.08.00.00"] <- colnames(Residuals)[1]
# total <- rbind(Residuals,forecast)
```

```{r}
# col = c(rep("orange", 7320), rep("green", 252))
# total <- cbind(total,col)
# head(total)
```

```{r}
# ggplot(total, aes(x=Date, y=residuals)) + 
